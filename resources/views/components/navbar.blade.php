                    ...
                    @if (Auth::check())
                        <el-dropdown class="relative ml-3">
                            <button
                                ...
                                <img src="{{ Auth::user()->avatar ? asset('storage/' . Auth::user()->avatar) : asset('img/default.jpg') }}"
                                    alt="{{ Auth::user()->name }}"
                            ...
                            <el-menu anchor="bottom end" popover
                                ...
                                <a href="/profile"
                                    ...
                                    profile</a>
                                <a href="/dashboard"
                                    class="block px-4 py-2 text-sm text-gray-300 focus:bg-white/5 focus:outline-hidden">Dashboard</a>
                                <form method="POST" action="/logout">
                                    @csrf
                                    <button type="submit"
                                        class="block w-full text-start px-4 py-2 text-sm text-gray-300 focus:bg-white/5 focus:outline-hidden cursor-pointer">Logout</button>
                                </form>
                            </el-menu>
                        </el-dropdown>
                    @else
                        <a href="/login" class="ml-4 flex items-center md:ml-6 text-sm text-white font-medium">Log
                            in</a>
                        <a href="/register"
                            class="ml-4 flex items-center md:ml-6 text-sm text-white font-medium">Register</a>
                    @endif
                </div>
            </div>
            ...
            @if (Auth::check())
                ...
                <div class="mt-3 space-y-1 px-2">
                    ...
                    <form method="POST" action="/logout">
                        @csrf
                        <button type="submit"
                            class="block w-full text-start rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-white/5 hover:text-white cursor-pointer">Logout</button>
                    </form>
                </div>
            @else
                <div class="my-3 space-y-1 px-2">
                    <a href="/login" class="block ml-4 items-center md:ml-6 text-sm text-white font-medium py-2">Log
                        in</a>
                    <a href="/register"
                        class="block ml-4 items-center md:ml-6 text-sm text-white font-medium py-2">Register</a>
            @endif
        </div>
    </el-disclosure>
</nav>
