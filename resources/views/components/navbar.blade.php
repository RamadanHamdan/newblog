<nav class="relative bg-gray-800 after:pointer-events-none after:absolute after:inset-x-0 after:bottom-0 after:h-px after:bg-white/10" x-data="{ isOpen: false }">
            ...
            <el-dropdown class="relative ml-3">
              ...
              <el-menu anchor="bottom end" popover class="w-48 origin-top-right rounded-md bg-gray-800 py-1 outline-1 -outline-offset-1 outline-white/10 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
                ...
                <form method="POST" action="/logout">
                  @csrf
                <button type="submit" class="block w-full text-start px-4 py-2 text-sm text-gray-300 focus:bg-white/5 focus:outline-hidden cursor-pointer">Logout</button>
                </form>
              </el-menu>
            </el-dropdown>
            @else
            <a href="/login" class="ml-4 flex items-center md:ml-6 text-sm text-white font-medium">Log in</a>
            <a href="/register" class="ml-4 flex items-center md:ml-6 text-sm text-white font-medium">Register</a>
            @endif
          </div>
        </div>
    ...
    <el-disclosure id="mobile-menu" hidden class="block md:hidden">
      ...
      <div class="border-t border-white/10 pt-4 pb-3">
        ...
        <div class="mt-3 space-y-1 px-2">
          ...
          <form method="POST" action="/logout">
                  @csrf
                <button type="submit" class="block w-full text-start rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-white/5 hover:text-white cursor-pointer">Logout</button>
                </form>
        </div>
        @else
        <div class="my-3 space-y-1 px-2">
          <a href="/login" class="block ml-4 items-center md:ml-6 text-sm text-white font-medium py-2">Log in</a>
          <a href="/register" class="block ml-4 items-center md:ml-6 text-sm text-white font-medium py-2">Register</a>
        @endif
      </div>
    </el-disclosure>
  </nav>